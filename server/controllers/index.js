...
// search for dog and update found dog's age (increment by 1)
const updateDog = async (req, res) => {
  ...
  try {
    const dog = await Dog.findOneAndUpdate({ name }, { $inc: { age: 1 } }, { returnDocument: 'after' }).lean().exec();
    if (!dog) {
